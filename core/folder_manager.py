"""Folder Manager with unlock cache - Versão Simplificada"""
...
class FolderManager:
    ...
    def get_folders(self) -> List[Dict]:
        """Get all folders"""
        folders = self.fm.get_folders(self.username)
        for folder in folders:
            folder["is_unlocked"] = folder["name"] in self.unlocked_folders
        return folders
    ...
    def unlock_folder(self, folder_name: str, password: str) -> bool:
        ...
        if self.crypto.verify_password(password, meta["password_hash"]):
            self.unlocked_folders.add(folder_name)
            self.folder_passwords[folder_name] = password
            print(f"[DEBUG] Folder '{folder_name}' unlocked")
            return True
        return False
