"""Note Manager - Handles note operations with encryption"""
...
class NoteManager:
    ...
    def __init__(self, username: str, master_password: str, file_manager: FileManager, crypto: CryptoManager):
        ...
        self.crypto = crypto
        self._master_key, _ = self.crypto.derive_key(master_password)

    def _get_folder_key(self, folder_name: str, folder_password: Optional[str] = None) -> bytes:
        """
        Get encryption key for a folder.
        For vault: uses master password key
        For protected folders: uses folder password
        For regular folders: uses master password key
        """
        # Vault sempre usa a senha mestre
        ...
        # Pasta não protegida ou não existe (vai ser criada)
        return self._master_key
    ...
    def create_note(self, folder_name: str, title: str, content: str, folder_password: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            key = self._get_folder_key(folder_name, folder_password)
            
            encrypted_content = self.crypto.encrypt(content, key) if content else ""
    ...
    def get_note(self, folder_name: str, note_id: str, folder_password: Optional[str] = None) -> Optional[Dict]:
        ...
        try:
            ...
            if note["content"]:
                content = self.crypto.decrypt(note["content"], key)
            else:
                content = ""
    ...
    def move_note(self, note_id: str, from_folder: str, to_folder: str, 
        ...
        """Move a note from one folder to another, re-encrypting if needed"""
        try:
